...
const GoldBtn = ({ label, setOpenModal }: { label: string, setOpenModal: React.Dispatch<React.SetStateAction<boolean>> }): ReactNode => {
    return (
        <div onClick={() => setOpenModal(prev => false)} className='flex justify-center items-center gap-2 w-[225px] h-[45px] bg-[color:var(--goldColor)] rounded-3xl hover:cursor-pointer'>
